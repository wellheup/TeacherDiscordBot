...
def test_uncomplete_entry(client):
        # Assume 'Edited Book Title test' entry is currently completed
        rv = client.get("/syllabus")
        data = rv.data.decode("utf-8")
        start_index = data.find("Edited Book Title test")
        if start_index == -1:
        ...
        # Find the uncomplete form (action="/uncomplete") for this entry
        uncomplete_form_start = data.find('action="/uncomplete', start_index)
        if uncomplete_form_start == -1:
                raise AssertionError("The uncomplete form was not found for the completed entry")

        unique_id_start = data.find('name="unique_id" value="', uncomplete_form_start) + len(
                'name="unique_id" value="'
        )
        unique_id_end = data.find('"', unique_id_start)
        unique_id = data[unique_id_start:unique_id_end].strip()

        # Post the uncomplete data to the /uncomplete route
        data_uncomplete = {"unique_id": unique_id}
        rv = client.post("/uncomplete", data=data_uncomplete, follow_redirects=True)
        assert rv.status_code == 200

        # Check that the entry now shows 'Mark Book Complete' instead of a date
        rv = client.get("/syllabus")
        assert b"Edited Book Title test" in rv.data
        data = rv.data.decode("utf-8")
        start_index = data.find("Edited Book Title test")
        section_end = data.find("Non-fiction", start_index)
        assert "Mark Book Complete" in data[start_index:section_end]
