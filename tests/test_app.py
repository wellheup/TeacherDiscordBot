...
def test_edit_entry(client):
	...
	unique_id_value_start = data.find('unique_id">', unique_id_start) + len('unique_id">')
...
def test_complete_entry(client):
	...
	# Check if the 'Mark Book Complete' button is no longer on the syllabus page for the completed entry
	rv = client.get('/syllabus')
	data = rv.data.decode('utf-8')
...
def test_delete_entry(client):
	# Assume 'Edited Book Title test' entry already exists and find its unique_id
	rv = client.get('/syllabus')
	data = rv.data.decode('utf-8')
	start_index = data.find('Edited Book Title test')
	if start_index == -1:
...
def test_add_bug(client):
	...
	data = {
		'description': 'New Bug Report test',
		'added_by': 'Tester test'
	}
...
def test_delete_bug(client):
	...
	if start_index == -1:
		raise AssertionError("The 'New Bug Report test' entry was not found in the bugs")
