...
async def update(
    ctx, 
    book_id : int = commands.parameter(default=None, description="The id of the book to update"), 
    column : str = commands.parameter(default=None, description="The column to update"), 
    new_value : str = commands.parameter(default=None, description="The new value for the column")
):
    db: Session = SessionLocal()
    is_demo = "demo" in ctx.channel.name
    try:
        if item.isdigit() and int(book_id) > 0:
