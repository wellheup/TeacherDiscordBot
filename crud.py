...
def get_current_assignment(db: Session, is_demo: bool):
        try:
                if is_demo:
                        ...
                        db_Assignment = (
                                db.query(Assignments).order_by(Assignments.assignment_id.desc()).first()
                        )
                return db_Assignment
        except Exception as e:
                db.rollback()  # Rollback on error
...
def get_pretty_columns(db: Session):
        columns = {
                ...
                "author": "Author",
                "series": "Series",
                "num_in_series": "Volume",
                "date_added": "Added On",
                "is_completed": "Done?",
                "added_by": "Added By",
                "season": "Season",
                "is_extra_credit": "Extra",
                "date_completed": "Completed On",
                "up_votes": "👍",
                "down_votes": "👎",
                "genre": "Genre",
        }
...
def complete_book(db: Session, book_name: str, is_demo: bool):
        ...
        try:
                if is_demo:
                        db_book = (
                                db.query(DemoSyllabus).filter(DemoSyllabus.book == book_name).first()
                        )
                else:
...
def uncomplete_book(db: Session, unique_id: int, is_demo: bool):
        try:
                if is_demo:
                ...
                db_book.is_completed = False
                db_book.date_completed = None
                db.commit()
                db.refresh(db_book)
                return db_book
        except Exception as e:
                db.rollback()
                print(f"Error uncompleting book: {e}")
                raise
                ...
                if is_demo:
                        return (
                                db.query(
                                        ...
                                        DemoSyllabus.author,
                                        DemoSyllabus.series,
                                        DemoSyllabus.num_in_series,
                                        DemoSyllabus.unique_id,
                                        DemoSyllabus.is_completed,
                                )
                                .filter(DemoSyllabus.is_completed == True)
                                .order_by(
                                        DemoSyllabus.author, DemoSyllabus.series, DemoSyllabus.num_in_series
                                )
                                .all()
                        )
                else:
...
def get_series(db: Session, series: str, is_demo: bool):
        if is_demo:
                ...
                return (
                        db.query(
                                Syllabus.book, Syllabus.author, Syllabus.series, Syllabus.num_in_series
                        )
...
def remove_id(db: Session, book_id: int, is_demo: bool):
        try:
                if is_demo:
                        db_book = (
                                db.query(DemoSyllabus).filter(DemoSyllabus.unique_id == book_id).first()
                        )
                else:
...
def update_book(
        db: Session, book_name: str, column: str, new_value: str, is_demo: bool
):
        try:
                if is_demo:
                        db_book = (
...
def update_database(
        ...
        try:
                if is_demo:
                        db_book = (
                                db.query(DemoSyllabus)
                                .filter(DemoSyllabus.unique_id == unique_id)
                                .first()
                        )
                else:
                        db_book = db.query(Syllabus).filter(Syllabus.unique_id == unique_id).first()
...
def get_books_by_author(db: Session, author: str, is_demo: bool):
        try:
                if is_demo:
                        return (
                                db.query(DemoSyllabus.series, DemoSyllabus.book)
                        ...
                        return (
                                db.query(Syllabus.series, Syllabus.book)
